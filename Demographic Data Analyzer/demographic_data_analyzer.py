...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(len(df['education'][df['education'] == 'Bachelors'])/len(df['education']) * 100,1)
    #percentage_bachelors = len(df['education'][(df['education'] == 'Bachelors') | (df['education'] == 'Masters') | (df['education'] == 'Doctorate')])/len(df['education'])
    ...
    higher_education_rich = round(100 - 100 * higher_education.loc[df['salary'] == '<=50K','salary'].shape[0] /higher_education.shape[0],1)
  
    lower_education_rich = round(100 - 100 * lower_education.loc[df['salary'] == '<=50K','salary'].shape[0] / lower_education.shape[0],1)
    ...
    country_ratio = df.groupby(['native-country'])['salary'].apply(lambda x : (x != '<=50K').sum()) / df['native-country'].value_counts() * 100
